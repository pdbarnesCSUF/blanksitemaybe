<?php
/*
 * @file admin_reset_picture.php
 * @desc Admin remove profile picture (ajax)
 */
/*
INPUT
...
{
	id_verbose('PAGE:ajax/admin/admin_reset_picture.php');
	id_init();
	//do something
		...
		if(id_account_isadmin())
		{
			...
			if (isset($_POST['userid']))
			{
				...
				if (id_userid_valid($_POST['userid']))
				{
					//id set
					//----------DO STUFF----------
					id_verbose("change pic");

					$resultpicture = id_profile_edit_picture($_POST['userid'],false);
					$responsearr['data']= $resultpicture;
					if ($resultpicture)
